...
const artifacts = require('../../../build/contracts/AcademicRecords.json');
...
export class Web3Service {
  ...
  async getContractInstance(): Promise<any> {
    ...
    const academicRecordsContract = contract(artifacts);
    academicRecordsContract.setProvider(this.web3.currentProvider);
    try {
      const academicRecordsInstance = await academicRecordsContract.deployed();
      return academicRecordsInstance;
    } catch (error) {
      console.log(error);
      throw new Error('Academic Records contract has not been deployed to detected network.');
    }
  }
